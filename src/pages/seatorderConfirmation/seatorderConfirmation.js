...
Page({
  ...
  getThemeName:function(res){//res  返回今天
    console.log(res)
          let timeArr=res.time.split('-');//分割字符  ==["2018", "07", "16"]
          let month=timeArr[1];//7
          let day=timeArr[2];//16
          // if(month<10){
          //   month=`0${month}`
          // }
          // if(day<10){
  ...
  getNowRangeTime:function(){
    ...
    //过滤已过去的时间
    let now = new Date();
    let hours = now.getHours();//当前的小时
    let minutes = now.getMinutes();//当前的分钟
