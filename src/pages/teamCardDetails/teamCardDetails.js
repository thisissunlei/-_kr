...
Page({
    ...
    getTeamCardDetail() {
        app.getRequest({
            ...
            success: res => {
                res.data.data.remainAmount = res.data.data.remainAmount.toString().replace(/(\d{1,3})(?=(\d{3})+$)/g,'$1,')
                res.data.data.faceValue = res.data.data.faceValue.toString().replace(/(\d{1,3})(?=(\d{3})+$)/g,'$1,')
                res.data.data.cost = res.data.data.cost.toString().replace(/(\d{1,3})(?=(\d{3})+$)/g,'$1,')
                ...
                if ( !!res.data.data.usetotal && !!res.data.data.lastUseTime ) {
                    res.data.data.lastUseTime = this.setTime(res.data.data.lastUseTime, 'last')
                }
    ...
    sure() {
        if ( !!this.data.changeNametext.trim() ) {
            app.getRequest({
                url: app.globalData.KrUrl + "api/gateway/kmteamcard/teamcard/updatecardname",
                method: "POST",
                data: {
                    cardId: this.cardId,
